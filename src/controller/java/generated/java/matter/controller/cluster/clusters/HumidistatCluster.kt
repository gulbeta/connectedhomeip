...
class AttributeListAttribute(
  ...
  suspend fun setSettings(userSetpoint: UByte?
      ,mode: UByte?
      ,mistType: UByte?
      ,continuous: Boolean?
      ,sleep: Boolean?
      ,optimal: Boolean?
      ,condPumpEnabled: Boolean?
    ...
    userSetpoint?.let {
      tlvWriter.put(ContextSpecificTag(TAG_USER_SETPOINT_REQ), userSetpoint)
    }
    ...
    mistType?.let {
      tlvWriter.put(ContextSpecificTag(TAG_MIST_TYPE_REQ), mistType)
    }

    val TAG_CONTINUOUS_REQ: Int = 3
    continuous?.let {
      tlvWriter.put(ContextSpecificTag(TAG_CONTINUOUS_REQ), continuous)
    }

    val TAG_SLEEP_REQ: Int = 4
    sleep?.let {
      tlvWriter.put(ContextSpecificTag(TAG_SLEEP_REQ), sleep)
    }

    val TAG_OPTIMAL_REQ: Int = 5
    optimal?.let {
      tlvWriter.put(ContextSpecificTag(TAG_OPTIMAL_REQ), optimal)
    }

    val TAG_COND_PUMP_ENABLED_REQ: Int = 6
    condPumpEnabled?.let {
      tlvWriter.put(ContextSpecificTag(TAG_COND_PUMP_ENABLED_REQ), condPumpEnabled)
    }    
